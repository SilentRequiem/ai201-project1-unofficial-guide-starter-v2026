...
```
The wait times at Kestrel Commons are 20 to 25 minutes between 12:15 and 1:00 (dining_kestrel_commons.txt and dining_kestrel_commons_followup.txt).
```
...
```
Based on `course_cs_210_exams.txt`, the midterms for CS 210 are curved, while the final is not.
```
...
```
According to `course_cs_210_exams.txt`, the midterms for CS 210 Data Structures are curved, while the final is not.
```
...
```
Based on the documents, the midterms for CS 210 are curved, while the final is not. This information comes from the file `course_cs_210_exams.txt`.
```
...
```
Based on the documents, the campus shuttle runs every 40 minutes on weekends. 
