...
```
Based on the documents, the midterms are curved, but the final is not (`course_cs_210_exams.txt`).
```
...
```
Based on the provided documents, the midterms for CS 210 are curved, but the final is not (source: `course_cs_210_exams.txt`).
```
...
```
Based on the documents, the midterms for CS 210 are curved, but the final is not (course_cs_210_exams.txt).
```
...
```
The campus shuttle runs a loop every 40 minutes on weekends (transit_shuttle.txt).
```
