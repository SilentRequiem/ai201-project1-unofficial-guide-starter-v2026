...
```
Wait times at Kestrel Commons are 20 to 25 minutes between 12:15 and 1:00 (dining_kestrel_commons.txt and dining_kestrel_commons_followup.txt).
```
...
```
According to `course_cs_210_exams.txt`, the midterms are curved, but the final is not.
```
...
```
Based on the documents, the midterms are curved, but the final is not (source: course_cs_210_exams.txt).
```
...
```
The campus shuttle runs a loop every 40 minutes on weekends. (Source: transit_shuttle.txt)
```
