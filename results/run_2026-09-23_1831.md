...
```
Between 12:15 and 1:00, wait times at Kestrel Commons are 20 to 25 minutes. 
...
```
Based on the provided documents, the midterms for CS 210 are curved, while the final is not (course_cs_210_exams.txt).
```
...
```
Based on the provided documents, the midterms for CS 210 are curved, but the final is not (from course_cs_210_exams.txt).
```
